...
export let options = {
  ...
  thresholds: {
    http_req_duration: ['p(95)<5000'],  // 95% de las peticiones deben estar debajo de 5 segundos
...
export function adminFlow() {
  ...
  if (loginAdministrador.status === 200) {
    ...
    check(registrarConductorFijo, {
      'registro conductor fijo status 200 or 400': (r) => r.status === 200 || r.status === 400,
    ...
    check(registrarConductorReemplazo, {
        'registro conductor reemplazo status 200 or 400': (r) => r.status === 200 || r.status === 400,
    ...
    check(reportes, {
        ...
        'mensaje esperado reporteo': (r) => r.status === 200 || (r.status === 400 && (r.json('msg_historial_reemplazo') !== undefined))
    });
