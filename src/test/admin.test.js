...
// Configuración inicial para la base de datos - Verficacoin de la conexion de las pruebas con la base de datos
...
describe('Pruebas de rutas del administrador', () => {
  ...
  test('Debe listar todos los conductores registrados', async () => {
    const token = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJlbWFpbCI6Im1pcmVnYXJjaWEyMDVAZ21haWwuY29tIiwicm9sZSI6ImFkbWluIiwiaWF0IjoxNzM4MTE2ODI2LCJleHAiOjE3MzgxMjA0MjZ9.FVIhqvi2W39UZha2jIC0ppNuSnycV2Rb8mzvOcng4Sg'; // Genera un token válido para pruebas
...
// Prueba: Eliminar un conductor
  test('Debe eliminar un conductor por su ID', async () => {
    ...
    const conductorId = '67998f3a3729c48a2e556ee4'; // ID del conductor existente para pruebas
...
//-----------------------------------------------------------------------------------------------//
// Prueba: Actualizar La Ruta y sector del conductor por ID
...
//-----------------------------------------------------------------------------------------------//
  // Vizualizar el perfil del administrador 
  test('Debe vizualizar el perfil del Administrador', async () => {
        const token = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJlbWFpbCI6Im1pcmVnYXJjaWEyMDVAZ21haWwuY29tIiwicm9sZSI6ImFkbWluIiwiaWF0IjoxNzM4MTE1MjA0LCJleHAiOjE3MzgxMTg4MDR9.eYrMMT6MrbBNfFwTNT01CKYQ472FqJjJpRcFekqd2Mc'; // Token válido

        const response = await request(app)
            .get(`/api/visualizar/perfil/admin`)
            .set('Authorization', `Bearer ${token}`) // Enviar el token de autenticación
            .send({
